#update tambahan produk biosolar b5 untuk print realisasi
import sys
import pandas as pd
import numpy as np
import xlrd
...
def dataLO(thisHour,textPath5):
    ...
    plt = sum(listPlt)
    bio5= sum(listBio5)
    totalitas = premium + bio + sol + dexl + PDex + px + pxt + plt + bio5
    ...
    bio5 = format(int(bio5), ',d')
    totalitas = format(int(totalitas), ',d')
    sumDataLo = '\n\n\t--Realisasi Pukul {10}.00--\n\nPREMIUM\t\t:\t{0}\nBIOSOLAR\t:\t{1}\nSOLAR\t\t:\t{2}\nDEXLITE\t\t:\t{3}\nPERTAMINA-DEX\t:\t{4}\nPERTAMAX\t:\t{5}\nPERTAMAX-TURBO\t:\t{6}\nPERTALITE\t:\t{7}\nBIOSOLAR B5\t:\t{8}\nTOTAL\t\t:\t{9}\n'.format(
        premium, bio, sol, dexl, PDex, px, pxt, plt,bio5, totalitas, thisHour)
    print(sumDataLo)
