...
#from getByMeterBYL(meterUnion.insert(12,None),meterUnion.insert(16,None) to below
#update getByMeterBYL(meterUnion.insert(10,None),meterUnion.insert(12,None),meterUnion.insert(14,None),meterUnion.insert(18,None))
#update getDataLOBYL(dLOBio5,dLO)
#update getCarrierBYL(schedBio5,metBio5,eoD)
#update getExcelBYL(SUMMARY INSERTION:[for i in range(8, 33): # update 31 to 33,lapSum[22][7].value = dLO['BIOSOLAR B5'],lapSum[22][8].value = eoD['eodBio5']])
#update getExcelBYL(THRUPUT INSERTION:)
'''
    for i in range(17, 22): #update 20 to 22 +2
        if lapSum.cell(row=i, column=5).value != None:
            sumSol.append(lapSum.cell(row=i, column=5).value - lapSum.cell(row=i, column=4).value)
    for i in range(23, 27): #update 21 to 23, 24 to 27
        if lapSum.cell(row=i, column=5).value != None:
            sumFame.append(lapSum.cell(row=i, column=5).value - lapSum.cell(row=i, column=4).value)
    ByMeter
    lapTru[18][2].value = sum(sumSol)#solar #update 17 to 18
    lapTru[21][2].value = eoD['metBio5']#new

    BySchedule
    lapTru[20][1].value = eoD['eodBIOSOLAR']#biosolar #update 19 to 20
    lapTru[21][1].value = eoD['eodBio5']#new

    ByLO
    lapTru[19][3].value = dLO['BIOSOLAR']#biosolar #update 19 to 20
    lapTru[21][3].value = dLO['BIOSOLAR B5']# new
'''
import openpyxl
import os
import csv
...
def getSumTempSite(x):
    if x == 'BLG':
        filePath = '../../template/thrusum/BLG/LAPORAN SUMMARY 23 Juli 2018.xlsx'
        return filePath
...
def getThruTempSite(x):
    if x == 'PLP':
        ...
        filePath = '../../template/thrusum/PLP/Laporan Thruput 23072018.xlsx'
        return filePath
    elif x == 'BLG':
        filePath = '../../template/thrusum/BLG/LAPORAN THRUPUT 23 Juli 2018.xlsx'
        return filePath
    elif x == 'BYL':
...
def getByMeterBYL(csvFilePath):
    ...
    for i in prodPx:
        meterUnion.append(i[3])
    #adjust the column
    meterUnion.insert(10,None)
    meterUnion.insert(12,None)
    meterUnion.insert(14,None)
...
def getByMeterBLG(csvFilePath):
    ...
    meterUnionOp.insert(15,None)
    meterUnionOp.insert(16,None)
    meterUnionOp.insert(21,None)
    meterUnionOp.insert(23,None)
    meterUnionOp.insert(26,None)
    meterUnionOp.insert(27,None)
    meterUnionOp.insert(28,None)
    meterUnionOp.insert(29,None)
    meterUnionOp.insert(30,None)
    ...
    meterUnionCl.insert(15,None)
    meterUnionCl.insert(16,None)
    meterUnionCl.insert(21,None)
    meterUnionCl.insert(23,None)
    meterUnionCl.insert(26,None)
    meterUnionCl.insert(27,None)
    meterUnionCl.insert(28,None)
    meterUnionCl.insert(29,None)
    meterUnionCl.insert(30,None)
...
def getDataLOPLP(dLOFilePath):
    ...
    for i in range(3, dataLO.nrows):
        if dataLO.cell_value(i,12) == 'PREMIUM':
            ...
            listBio5.append(dataLO.cell_value(i,14))
...
def getCarrierPLP(csvCarrier):
    ...
    eoD = {'eodPREMIUM':sum(eodPre),
           ...
           'eodSOLAR':sum(eodSol),
           'metSOLAR':sum(metSol),
           'eodBIOSOLAR5':sum(eodBio5),
           'metBIOSOLAR5':sum(metBio5), 
...
def getExcelBLG(byMeter,byLo,bySched,dispet,patra,ptm):
    ...
    lapSum[23][7].value= dLO['BIOSOLAR B5'] #biosolar b5
    lapSum[28][7].value= dLO['PERTAMAX']  #pertamax 26 TO 28
    lapSum[30][7].value= dLO['PERTALITE']  #pertalite 28 TO 30
    lapSum[32][7].value= dLO['PERTAMAX TURBO'] #pertamax-turbo 30 TO 32
    lapSum[38][7].value= dLO['AVTUR'] #avtur 36 TO 38
    ...
    lapSum[23][8].value= eoD['eodBIOSOLAR5']#biosolar b5
    lapSum[28][8].value= eoD['eodPERTAMAX']  #pertamax 26 TO 28
    lapSum[30][8].value= eoD['eodPERTALITE']  #pertalite 28 TO 30
    lapSum[32][8].value= eoD['eodPERTAMAX-TURBO'] #pertamax-turbo 30 TO 32
    lapSum[38][8].value= eoD['eodAVTUR'] #avtur 36 TO 38
    ...
    for i in range(24,27): # 22 to 25 update 24 to 27
        sumFame.append(lapSum.cell(row=i,column=5).value - lapSum.cell(row=i,column=4).value)
    ...
    #Minus operation because of none type
    px1 = lapSum[27][4].value - lapSum[27][3].value #25 to 27
    px2 = lapSum[29][4].value - lapSum[29][3].value #27 to 29
    px3 = lapSum[31][4].value - lapSum[31][3].value #29 to 31
    sumPx= px1+px2+px3
    #Minus operation because of single operation
    sumPxt = lapSum[32][4].value - lapSum[32][3].value # 30 to 32
    #Minus operation because of single operation
    avt1 = lapSum[38][4].value - lapSum[38][3].value # 36 to 38
    avt2 = lapSum[39][4].value - lapSum[39][3].value # 37 to 39
    sumAvt = avt1 + avt2
    ...
    lapTru[24][5].value= sumSol#solarTotal 22 to 24
    lapTru[25][5].value= sum(sumFame)#fame 23 to 25
    ...
    lapTru[20][6].value= eoD['metBIOSOLAR5']#meterBiosolar5
    ...
    lapTru[20][3].value= dLO['BIOSOLAR B5']#biosolarB5
    ...
    lapTru[20][7].value=eoD['eodBIOSOLAR5']#biosolarB5
    ...
    #2. CHANGE THE NAME
    lapSum[52][1].value = dispet # 50 to 52
    lapSum[52][6].value = patra
    lapSum[52][11].value = ptm
    lapTru[37][0].value = dispet # 35 to 37
...
def getExcelBYL(byMeter,byLo,bySched,fileSum,dispet,patra,ptm):
    ...
    for i in range(8, 33): # update 31 to 33
    ...
    lapSum[22][7].value = dLO['BIOSOLAR B5']#biosolar B5
    lapSum[28][7].value = dLO['PERTAMAX']#pertamax 26 to 28
    lapSum[31][7].value = dLO['PERTALITE']#pertalite 29 to 31
    ...
    lapSum[22][8].value = eoD['eodBio5']#biosolar B5
    lapSum[28][8].value = eoD['eodPERTAMAX']#pertamax 26 to 28
    lapSum[31][8].value = eoD['eodPERTALITE']#pertalite 29 to 31
    # THRUPUT BYMETER PROCESSING
    sumPre=[]
    sumPx=[]
    sumSol=[]
    ...
    for i in range(8, 17): 
        sumPre.append(lapSum.cell(row=i, column=5).value - lapSum.cell(row=i, column=4).value)
    for i in range(27, 32):#update 25 to 27, 30 to 32 +2
    ...
    lapTru[18][2].value = sum(sumSol)#solar #update 17 to 18
    lapTru[20][2].value = eoD['metBIOSOLAR']#biosolar daru metBio #update 19 to 20
    lapTru[21][2].value = eoD['metBio5']#new
    lapTru[23][2].value = sum(sumFame)#fame #update 21 to 23
    # By Schedule
    ...
    lapTru[20][3].value = dLO['BIOSOLAR']#biosolar #update 19 to 20
    lapTru[21][3].value = dLO['BIOSOLAR B5']# new
    # FINISHING
    # 1. CHANGE THE DATE
    todays = getDateDash()
    ...
    # 2. CHANGE THE NAME
    lapSum[41][0].value = dispet
    lapSum[41][7].value = patra
    lapSum[41][11].value = ptm
...
def getExcelPLP(byMeter,byLo,bySched,fileSum,dispet,patra,ptm,*args):
    ...
    lapSum[46][9].value = dLO['BIOSOLAR B5']#BIOSOLAR B5	
    ...
    lapSum[46][8].value = eoD['eodBIOSOLAR5']#BIOSOLAR B5
    ...
    lapTru[8][1].value = eoD['eodBIOSOLAR5']#BIOSOLAR B5	
    lapTru[10][1].value = eoD['eodDEXLITE']#DEXLITE
    lapTru[18][1].value = eoD['eodPERTAMAX']#PERTAMAX
    lapTru[19][1].value = eoD['eodPERTALITE']#PERTALITE
    lapTru[20][1].value = eoD['eodPERTAMAX-TURBO']#PERTAMAX-TURBO
    lapTru[21][1].value = eoD['eodDEX']#PERTAMINA-DEX
    # byMeter
    lapTru[6][2].value = eoD['metSOLAR']#SOLAR
    lapTru[19][2].value = eoD['metPERTALITE']#PERTALITE
    lapTru[8][2].value = eoD['metBIOSOLAR5']#BIOSOLAR B5
    lapTru[10][2].value = eoD['metDEXLITE']#DEXLITE
    lapTru[5][2].value = '=%i-((45/100)*C19)' % sum(sumPre)#PREMIUM
    lapTru[18][2].value = '=%i-((55/100)*C19)' % sum(sumPx)#PERTAMAX
    lapTru[12][2].value = sum(sumFame)#FAME
    lapTru[17][2].value = sum(sumSol)#SOLAR TOTAL
    lapTru[20][2].value = sum(sumPxT)#PERTAMAX-TURBO
    #lapTru[20][2].value = '=20/100*C10'
    lapTru[21][2].value = '=%i-C22' % sumDex #PERTAMINA-DEX
    # by LO
    lapTru[5][3].value = dLO['PREMIUM']#PREMIUM
    lapTru[6][3].value = dLO['SOLAR']#SOLAR
    lapTru[7][3].value = dLO['BIOSOLAR']#BIOSOLAR
    lapTru[10][3].value = dLO['DEXLITE']#DEXLITE
    lapTru[18][3].value = dLO['PERTAMAX']#PERTAMAX
    lapTru[19][3].value = dLO['PERTALITE']#PERTALITE
    lapTru[20][3].value = dLO['PERTAMAX TURBO']#PERTAMAX-TURBO
    lapTru[21][3].value = dLO['PERTAMINA DEX']#PERTAMINA-DEX
    #lapTru['D11'] = '=0.8*D7'
    ...
    lapTru['a32'].value = dispet
    lapTru['b32'].value = patra
    lapTru['c32'].value = ptm
    lapTru['e32'].value = args[0]
    return f,g
